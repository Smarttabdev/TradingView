            ...
            if ($email && $password) {
                if (Auth::attempt(['email' => $email, 'password' => $password, 'active' => 1], true)) {
                    ...
                    if ($account_number && $broker) {
                        ...
                        if (!$account) {
                            $account = Accounts::create(['account_number' => $account_number, 'broker' => $broker, 'account_type' => $account_type]);
                    ...
                    $groupSettingUser = User::with('groupSetting')->find($user->id);
                    $groupSetting = $groupSettingUser->groupSetting;
                    return response()->json([
                        'response' => [
                            'api_status' => 1,
                            'code' => 200,
