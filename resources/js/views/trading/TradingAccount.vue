      ...
      <v-row>
        ...
        >
          ...
          <v-data-table
            ...
            :search="search"
            item-key="email"
            :server-items-length="accounts_total"
          ...
          >
            ...
            <template v-slot:item="props">
              <tr>
                ...
                <td>
                  <v-badge :value="false" :class="props.item.statusColor"
                    >{{ props.item.status }}
                  </v-badge>
                </td>
                ...
                <td>
                    ...
                    <v-tooltip bottom>
                      <template v-slot:activator="{ on }">
                        <v-btn
                          ...
                          icon
                          color="error"
                          @click="removeAccount(props.item.id)"
                        >
                  ...
                  <template v-else-if="props.item.status == 'COPY'">
                    <router-link
                      :to="{
                        name: 'copying-signal',
                        hash: '#' + props.item.account_number,
                      }"
                    >
                      <v-tooltip bottom>
                        <template v-slot:activator="{ on }">
        ...
        <copy-setting-dialog
          ...
          :form.sync="form"
          :formstep.sync="formstep"
          :isLoadingSettings="isLoadingSettings"
          :isFormSubmitting="isFormSubmitting"
          :prevStep="prevStep"
          :checkNextisDisabled="checkNextisDisabled"
    ...
    ...{
      ...
      saveSettings() {
        ...
        if (this.settingType == "copy") {
          axios
            .post(`${webServices.baseURL}/savecopysetting`, {
              ...this.form,
          ...
          axios
            .post(`${webServices.baseURL}/saveprovidesetting`, {
              ...this.form,
              ...this.selected_account,
            })
            .then(({ data }) => {
      ...
      changeSetting(account_number, broker, settingType) {
        ...
        if (settingType == "copy") {
          axios
            .post(`${webServices.baseURL}/copysetting`, this.selected_account)
            .then(({ data }) => {
              const { response } = data;
          ...
          axios
            .post(
              `${webServices.baseURL}/providesetting`,
              this.selected_account
            )
            .then(({ data }) => {
              const { response } = data;
      ...
      removeAccount(account_id) {
        axios
          .delete(`${webServices.baseURL}/accounts/${account_id}`)
          .then(({ data }) => {
            Vue.notify({
              group: "signals",
              type: "success",
          ...
          .catch(() => {
            Vue.notify({
              group: "signals",
              type: "error",
              text: "Can't remove this account",
            });
            Nprogress.done();
          });
      },
    ...
    ...{
      checkNextisDisabled() {
        switch (this.formstep) {
          case 6:
            if (!this.form.applyOnOffTime) return false;
