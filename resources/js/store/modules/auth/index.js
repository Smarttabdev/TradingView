...
const actions = {
    ...
    signInWithLaravelPassport(context, payload) {
        ...
        axios.post(`${webServices.baseURL}/auth/login`, user, { headers: { 'Content-Type': 'application/json' } })
            .then(response => {
                const { api_status, access_token, name, email, roles, avatar, group } = response.data.response;
                if (api_status) {
                    localStorage.setItem("access_token", access_token);
                    setTimeout(() => {
                        context.commit('loginUserSuccess', { name, email, roles, avatar, group });
                    }, 500);
                } else {
    ...
    changeGroupSettingAction(context, group) {
        context.commit('changeGroupSetting', group);
    }
}
...
const mutations = {
    ...
    loginUserSuccess(state, user) {
        ...
        if (user.roles.findIndex(role => role == 'ROLE_ADMIN') !== -1)
            router.push("/available-sources");
        else if (user.roles.findIndex(role => role == 'ROLE_MANAGER') !== -1)
            router.push("/users-list");
        else router.push("/available-sources");
    ...
    changeGroupSetting(state, group) {
        if (state.user) {
